...
public abstract class WkSerdeDtreePrimitiveMsgReaderEngine<
                        X,
                        QC extends WkSerdeDtreeOperationInputRuntimeCtrl<?,?,?>,
                        O extends WkSerdeDtreeMsgReader<X,?,?,?,?>>
    extends WkSerdeDtreePrimitiveMsgOperationEngine<QC,O>
{

  private static final Logger logger = LoggerFactory.getLogger(WkSerdeDtreePrimitiveMsgReaderEngine.class);

  protected WkSerdeDtreePrimitiveMsgReaderEngine(
    QC runtimeControl,
    O readingOperation) {
  ...
  protected void onBytesLeftToProcess() {
    if(operation().parentField().deserializedRequired()) {
      readBytes();
    } else {
      skipBytes();
    }
