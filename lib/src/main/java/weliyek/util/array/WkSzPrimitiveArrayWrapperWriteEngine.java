...
import weliyek.serialization.WkSzWriteEngine;
...
public abstract class WkSzPrimitiveArrayWrapperWriteEngine<
                        Y extends WkPrimitiveArrayBase<V, ?>,
                        V,
                        QC extends WkSzSequenceWritingRuntimeControl<?,?,?>>
        extends WkSzWriteEngine<
                        Y,
                        QC,
                        WkSzPrimitiveArraySerializerWriter<? extends Y,?,?,?,?>>
{

  private static final Logger logger = LoggerFactory.getLogger(WkSzPrimitiveArrayWrapperWriteEngine.class);

  private final int primitiveByteLength;
  private int start = -1;
  private int stop = 0;

    protected WkSzPrimitiveArrayWrapperWriteEngine(
      QC runtimeControl,
